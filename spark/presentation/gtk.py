...
import gtk
import os
from sparck.interfaces import IPresentationAgent
from sparck.event import EventBus
from sparck.agent import BaseAgent
from zope.interface import implements

pygtk.require("2.0")


class GtkBasePresentationAgent(BaseAgent):
    """
    ...
    def attach_slave(self, name, container, slave):
        """
        ...
        self.pack_slave(widget, container)
        self._slaves[name] = slave

    def pack_slave(self, widget, container):
        """
        Packs a slave window into the container identifed by container name.

        If the default pack behaviour does not suit your needs, you may
        override this method.

        @param widget:      The slave widget to attach in container.
        @param container:   The container widgte name to add slave to.
        """
        container = self._builder.get_object(container)

        if container is None:
            raise AttributeError("No container named %s" % container)
